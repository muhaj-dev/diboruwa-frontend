...
export const UserFoodOrders: React.FC = () => {
  const [showFoodTrackers, setShowFoodTrackers] = useState<boolean[]>([
    false,
    false,
    false,
    false,
  ...
  return (
    <div className="user_card_overflow">
      {[0, 1, 2, 3, 4, 5].map((sectionIndex) => (
        <div className="FoodTracker_container" key={sectionIndex}>
          ...
          {showFoodTrackers[sectionIndex] && <FoodTracker />}
        </div>
      ))}
    </div>
  );
